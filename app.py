...
try:
    growth_summary = get_growth_summary(raw_df, snapshots_df)

    if growth_summary.empty:
        st.info(
            "Growth analizi için henüz yeterli snapshot verisi yok. Bir cover için en az 2 snapshot gerekli."
        )
    else:
        ...
        available_columns = [
            col for col in preferred_columns if col in growth_summary.columns
        ]

        if available_columns:
            st.dataframe(growth_summary[available_columns], use_container_width=True)
        else:
    ...
    if cover_history.empty:
        ...
        chart_history["snapshot_date"] = pd.to_datetime(chart_history["snapshot_date"])
    ...
    st.warning(
        "metrics_snapshots.csv dosyası bulunamadı. Önce en az bir metric snapshot eklemelisin."
    )

except Exception as e:
    st.error(f"Growth Analytics yüklenirken bir hata oluştu: {e}")
